...
Thus far we have assessed model fit based on expectations and eyeballing of scatter plots and fitted regression lines.  To summarise lets look at each model from a residual vs actual and predicted vs actual perspective.  This might tease out further insights.

```{r}
resid_data <- data %>% 
  mutate(
    ols_agg_e = lm_agg_pred  - log_pb,
  ...
  select(-int_ols, -int_ts, -slp_ols, -slp_ts, -bayes1_pred, -bayes2_pred) %>% 
...
NO FORMAL MODEL COMPARISON OR DIAGNOSTICS OF THE MODEL FIT
