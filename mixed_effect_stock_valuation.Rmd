...
A couple of the OLS models have negative slopes, this is counter to expectation.  Consistent with the observations above, the Theil-Sen slopes are more compact or closer to each other.
...
We now look at multi-level models.  The expectation is that parameters estimated using this technique (as compared to independent OLS models) will experience shrinkage to a mean as the population data influences each groups coefficients.  The references and introductory quotes inform support this view.    

Of interest is the extent to which the shrinkage aligns coeffiecents with the Theil-Sen robust model.  
...
Compared to expectation, that was a little underwhelming.  There is essentially no difference between the mixed effects and individual OLS models.  Why is this so?  That question should be considered in light of the drivers of parameter shrinkage in multi-level models.  Shrinkage is driven by:  
...
One last point in relation to this model.  As defined, it estimates a correlation between the intercepts and slopes for each sector. Is this correlation a reasonable assumption to make in terms of model structure?  Lets explore that question.  The intercept represents the outcome when the predictor is zero.  A zero return on equity would imply that the market value of a business equal its accounting / book value (assuming the book value is fair).  An asset only has a premium over it's book value if it earns returns.  So in this case the P/B ratio is one and the log_pb is 0.  We have scaled the ROE so the numbers change but the intuition should remain.  Wilcox [5] at p.199 defines the slope as representing the investment horizon before the ROE reverts to its mean [^2].
...
The intercept is defined as the outcome (log_pb) when the predictor (roe) is zero.  When the predictor variable is standardised like we have just done, it's mean is zero.  So what is a reasonable expectation of the log_bp when roe is at it's mean?  Luckily the second plot above uses the standardised ROE and we can see this is associated with a log_pb of circa 1.25.  That converts to a P/B ratio of around 3.5 (the exponent of 1.25) which sounds reasonable.  

The model below therefore has a N(1.25, 1) prior for the intercept and N(2, 1.5) prior for the slope.
...
bayes1 <- ulam(
  alist(
    ...
    b ~ normal(2, 1.5),                         # prior for average slope
...
## Bayesian approach - Student t likelihood

Let's re-estimate the bayesian model using tighter priors and a student t likelihood for a more robust approach.  Will this model configuration turn those un-intuitive negative slopes positive?
...
Using the Student t distribution does not result in drastically different slope estimates (except sector 7 - Healthcare).  It may have reduced uncertainty (remember that is something we get with bayesian estimation) but we are not looking at that here.  

<br>

## Bayesian model with measurement error (student t) 

We now attempt to account for measurement error in the price to book ratio. It is well known that stock prices are more volatile than the underlying fundamentals.  It is reasonable to expect that any measurement error or random noise in this ratio scales with the volatility of the underlying stock price.

The model that follows is based on that per McElreath [6] p.493, and considers the true underlying P/B ratio a function of the observed ratio and an error component that scales with trailing price volatility.
...
Once again, not a large deviation in slope estimates across different techniques.  It is difficult to pin down why this is so without performing a slew of additional analysis.  I'm going to point out a two potential drivers:  

1. Omitted variable bias, the driver of the outlying data points is a factor not modeled, or  
...
Thus far we have assessed model fit based on expectations and eyeballing of scatterplots and fitted regression lines.  To summarise lets look at each model from a residual vs actual and predicted versus actual perspective.  This might tease out some further insight.
...
resid_data <- data %>% 
  mutate(
    ...
    bayes1_e  = bayes1_pred - log_pb,
    bayes2_e  = bayes2_pred - log_pb,
    bayes3_e  = bayes3_pred - log_pb
    ) %>% 
  select(log_pb:bayes3_e) %>% 
  pivot_longer(!log_pb, names_to = 'model_type', values_to = 'value') %>% 
...
```{r}
ggplot(
  resid_data[resid_data$data_type == 'residual', ], 
  aes(y = log_pb, x = value)
  ) + 
  geom_point(alpha = 0.3) +
  facet_wrap(vars(model_type)) +
...
WHAT WASN'T DONE AND COULD BE DONE.

NOTE RE MISSING PREDICTORS

THE ABOVE IGNORES PARAMETER ESTIMATE UNCERTAINTY FOR BAYESIAN MODELS

COULD HAVE LOOKED AT ICC TO DETERMINE SUITABILIY OF MLM  
...
https://mldscenter.maryland.gov/egov/Publications/ResearchSeries/Clustered%20Data,%20Are%20Multilevel%20Models%20Really%20Necessary.pdf
