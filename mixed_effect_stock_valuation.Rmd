...
This post will be an exploration of various types of regression models.  The intend is to gain an intuition for different techniques and the context for this will be assessing stock valuation. A particular focus will be on multi-level models and accounting for robustness in the presence of outliers.  
...
Regression techniques assessed will cover linear models estimated using ordinary least squares (on a pooled and un-pooled basis), multi-levels models (estimated using both frequentist and bayesian approaches).  As mentioedn above, on-parametric robust techniques will also get a look in.  Multi-level models will be extended to account for measurement error in predictor variables, a useful technique that Bayesian estimation allows us implement.  We will also model the response with a non-normal link functions, using the Student t distribution to account for fat tailed data points.

As alluded to in the quotes above, and already mentioned, the focus will be on multi-level models.  Why?  Multi-level models purport to provide robust results by virtue of partial pooling.  Partial pooling results in regularisation of parameter estimates.  Robust regression techniques also purport to provide robust results (it's all in the name isn't it).  We will see how they compare.   
...
Two approaches that have a similar aim but take different routes to model data.  It will be interesting to review the difference in fit between models that pool data and embed an underlying distributional assumption (ADD QUOTE FROM MCELREATH RE NORMAL AFTER CONDITIONING ON PREDICTORS), and those that are non-parametric and use robust techniques.  
...
Stocks with similar characteristics are grouped into sectors.  It is reasonable to expect that sectors may have different characteristics giving rise to differing relationships between ROE and valuation.  Different sectors may have different growth prospects and risk profiles for example. 

With this in mind, the plot below shows the same regression models estimated above (the GAM has been removed), except now applied individually to each sector.  
```{r}
...
Below, the intercepts and slopes for each of the individual linear models presented above are plotted.  
...
We now look at Multi level models.  The expectation here is that parameters estimated with this model (as compared to independent OLS models) will experience shrinkage to a mean as the population data influences each groups coefficients.  
...
Sectors that experienced the most shrinkage (the difference between the independent OLS and Multi-level models) are those that have fewer data points and / or the most extreme (furthest from average) parameter estimates pre shrinkage.  Sector 9 (????) falls into this category.
...
Using the Student t distribution does not result in drastically different slope estimates.  It may have reduced uncertainty (remember that is something we get with bayesian estimation) but we are not looking at that here.  

<br>

## Bayesian model with measurement error (student t) 

We now attempt to account for measurement error in the price to book ratio. We know that stock prices are more volatile than the underlying fundamentals (INSERT LINK REFERENCE).  It is reasonable to expect that any measurement error or random noise in this ratio scales with the volatility of the underlying stock price.

The model that follows is based on that per McElreath's text (p 493) and considers the true underlying P/B ratio is a function of the observed ratio and an error component that scales with trailing volatility.
...
OK.  Once again, not a large deviation in slope estimates across different techniques.  EXPAND COMMENTARY
